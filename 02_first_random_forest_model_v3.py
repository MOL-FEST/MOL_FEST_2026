#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Ders 1B — İki Veri Setinde İlk Random Forest Baseline

# %% [markdown]
# # Ders 1B — İki Veri Setinde İlk Random Forest Baseline
# 
# Her iki veri seti için ayrı Random Forest modeli eğitilir. Confusion matrix, ROC curve, metrik tablosu, tahmin dosyası ve model dosyası kaydedilir.
...
# %% [markdown]
# ## 7. İki veri setinde ilk Random Forest modeli
# 
# Bu bölümde her veri seti için ayrı Random Forest modeli eğitilir.  
# Bu modeller başlangıç referansı olarak kullanılacaktır.
# 
# Oluşacak model adları:
# - `model_ERa_BLA_rf_baseline`
# - `model_ERa_LUC_VM7_rf_baseline`

# %%
lesson_out = ensure_output_root() / "02_first_random_forest"
...
for dataset_key, data in features.items():
    info = data["info"]
    model_name = f"{info['model_prefix']}_rf_baseline"

    note(f"RF baseline eğitiliyor: {model_name}")

    model, metrics, y_pred, y_score, X_train, X_test, y_train, y_test, df_train, df_test = train_test_rf(data)
    ...
    save_prediction_table(lesson_out, model_name, df_test, data["smiles_col"], y_test, y_pred, y_score)
...
metrics_df = pd.DataFrame(rows).sort_values("ROC", ascending=False)
metrics_df.to_csv(lesson_out / "rf_baseline_metrics_both_datasets.csv", index=False)
display(metrics_df)
