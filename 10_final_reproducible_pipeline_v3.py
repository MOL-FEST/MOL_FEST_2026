#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Ders 5B — Final Reproducible Pipeline

# %% [markdown]
# # Ders 5B — Final Reproducible Pipeline
# 
# Baseline, feature engineering, feature selection, ensemble ve sampling stratejileri tek final tabloda karşılaştırılır.
...
# %% [markdown]
# ## 7. Final karşılaştırma
# 
# Bu bölüm önceki adımları tek tabloda toplar.
# 
...
lesson_out = ensure_output_root() / "10_final_pipeline"
models_dir = lesson_out / "saved_models"
models_dir.mkdir(parents=True, exist_ok=True)

loaded = load_all_datasets()

def resample_to_ratio_final(y_train, ratio):
...
def evaluate_strategy(dataset_key, raw_data, strategy_name, feature_set, model, top_anova_k=None, balanced_oversampling=False):
    X, y, names, df_valid = build_features(raw_data["df"], raw_data["y"], raw_data["smiles_col"], feature_set)

    X_train, X_test, y_train, y_test, df_train, df_test = train_test_split(
        X, y, df_valid, test_size=TEST_SIZE, stratify=y, random_state=RANDOM_STATE
    )

    if top_anova_k is not None:
        F, _ = f_classif(X_train, y_train)
        idx = np.argsort(np.nan_to_num(F))[::-1][:min(top_anova_k, X.shape[1])]
        X_train = X_train[:, idx]
        X_test = X_test[:, idx]
        names = [names[i] for i in idx]

    if balanced_oversampling:
        idx_res = resample_to_ratio_final(y_train, ratio=1.0)
        X_train = X_train[idx_res]
        y_train = y_train[idx_res]
    ...
    model_name = f"{raw_data['info']['model_prefix']}_{strategy_name}"
    metrics.update({
        "Dataset": dataset_key,
    ...
    joblib.dump({"model": model, "feature_names": names}, models_dir / f"{model_name}.joblib")
    return metrics

# %%
rows = []

for dataset_key, raw_data in loaded.items():
    note(f"Final stratejiler çalışıyor: {dataset_key}")

    rows.append(evaluate_strategy(dataset_key, raw_data, "RF_baseline_morgan", "morgan", make_rf()))
    rows.append(evaluate_strategy(dataset_key, raw_data, "RF_maccs_morgan", "maccs_morgan", make_rf()))
    rows.append(evaluate_strategy(dataset_key, raw_data, "RF_anova_top100", "maccs_morgan", make_rf(), top_anova_k=100))
    rows.append(evaluate_strategy(dataset_key, raw_data, "Voting_ensemble", "morgan", voting_model()))
    rows.append(evaluate_strategy(dataset_key, raw_data, "RF_balanced_oversampling", "morgan", make_rf(class_weight=None), balanced_oversampling=True))

final_df = pd.DataFrame(rows).sort_values(["Dataset", "ROC"], ascending=[True, False])
final_df.to_csv(lesson_out / "final_comparison_both_datasets.csv", index=False)
display(final_df)

for dataset_key in final_df["Dataset"].unique():
    sub = final_df[final_df["Dataset"] == dataset_key].sort_values("ROC", ascending=True)

    plt.figure(figsize=(10, 5))
    plt.barh(sub["Strategy"], sub["ROC"])
    plt.xlabel("ROC-AUC")
    plt.title(f"{dataset_key} — Final strateji karşılaştırması")
    plt.tight_layout()
    plt.savefig(lesson_out / f"{dataset_key}_final_strategy_roc.png", dpi=300, bbox_inches="tight")
    plt.show()

best = final_df.sort_values("ROC", ascending=False).iloc[0]
print("Final pipeline tamamlandı.")
print(f"Genel en yüksek ROC: {best['ModelName']} | ROC={best['ROC']:.3f}")
