...
# # Ders 5A — Class Imbalance, Oversampling ve Undersampling
# 
# Train set üzerinde farklı sınıf oranları oluşturulur ve Random Forest performansı karşılaştırılır.
...
# %% [markdown]
# ## 7. Oversampling ve undersampling
# 
# Bu bölümde train set üzerindeki sınıf oranı değiştirilir.  
# Test set değiştirilmez; çünkü test set gerçek performans ölçümü için doğal dağılımında kalmalıdır.

# %%
lesson_out = ensure_output_root() / "09_resampling"
...
def resample_to_ratio(y_train, ratio, method):
...
for dataset_key, data in features.items():
    ...
    for scenario, ratio in scenarios:
        for method in methods:
            run_name = f"{data['info']['model_prefix']}_{scenario}_{method}"
            note(f"Sampling: {run_name}")

            idx = resample_to_ratio(y_train, ratio, method)
            X_res = X_train[idx]
            y_res = y_train[idx]
            ...
            metrics.update({
                ...
                "TrainPositiveAfter": int(np.sum(y_res == 1)),
                "TrainNegativeAfter": int(np.sum(y_res == 0))
            })
            rows.append(metrics)

            joblib.dump({"model": model, "feature_names": data["feature_names"]}, models_dir / f"{run_name}.joblib")

            ConfusionMatrixDisplay.from_predictions(y_test, y_pred, display_labels=["class 0", "class 1"])
            plt.title(run_name)
            plt.tight_layout()
            plt.savefig(lesson_out / f"{run_name}_confusion_matrix.png", dpi=300, bbox_inches="tight")
            plt.show()

            print(f"ROC={metrics['ROC']:.3f}, Recall={metrics['Recall']:.3f}, Specificity={metrics['Specificity']:.3f}, Precision={metrics['Precision']:.3f}")

result_df = pd.DataFrame(rows).sort_values(["Dataset", "BalancedAccuracy"], ascending=[True, False])
result_df.to_csv(lesson_out / "resampling_metrics_both_datasets.csv", index=False)
display(result_df)
