#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Ders 2A — İki Veri Setinde 10 Klasik Model

# %% [markdown]
# # Ders 2A — İki Veri Setinde 10 Klasik Model
# 
# On farklı klasik makine öğrenmesi modeli iki veri setinde ayrı ayrı eğitilir ve aynı metriklerle karşılaştırılır.
...
# %% [markdown]
# ## 7. İki veri setinde 10 klasik model
# 
# Bu bölümde model aileleri karşılaştırılır.  
# Aynı veri ve aynı split üzerinde farklı modelleri denemek, model seçimini daha kontrollü hale getirir.
...
def scaled(estimator):
    return Pipeline([("variance", VarianceThreshold(0.0)), ("scaler", StandardScaler()), ("model", estimator)])

def tree(estimator):
    return Pipeline([("variance", VarianceThreshold(0.0)), ("model", estimator)])

def get_10_models():
    return {
        "LogisticRegression": scaled(LogisticRegression(max_iter=2000, class_weight="balanced", solver="liblinear")),
        "kNN": scaled(KNeighborsClassifier(n_neighbors=7, weights="distance")),
        "LinearSVM": scaled(CalibratedClassifierCV(LinearSVC(class_weight="balanced", random_state=RANDOM_STATE), cv=3)),
        "RBF_SVM": scaled(SVC(C=3.0, kernel="rbf", gamma="scale", class_weight="balanced", probability=True, random_state=RANDOM_STATE)),
        "RandomForest": tree(RandomForestClassifier(n_estimators=300, class_weight="balanced_subsample", random_state=RANDOM_STATE, n_jobs=-1)),
        "ExtraTrees": tree(ExtraTreesClassifier(n_estimators=300, class_weight="balanced", random_state=RANDOM_STATE, n_jobs=-1)),
        "GradientBoosting": tree(GradientBoostingClassifier(random_state=RANDOM_STATE)),
        "HistGradientBoosting": tree(HistGradientBoostingClassifier(random_state=RANDOM_STATE)),
        "GaussianNB": tree(GaussianNB()),
        "DecisionTree": tree(DecisionTreeClassifier(max_depth=8, class_weight="balanced", random_state=RANDOM_STATE)),
    }

lesson_out = ensure_output_root() / "03_train_10_models"
...
for dataset_key, data in features.items():
    ...
    for model_label, model in get_10_models().items():
        model_name = f"{data['info']['model_prefix']}_{model_label}"
...
metrics_df = pd.DataFrame(rows).sort_values(["Dataset", "ROC"], ascending=[True, False])
metrics_df.to_csv(lesson_out / "ten_model_metrics_both_datasets.csv", index=False)
display(metrics_df)

# %% [markdown]
# ## 8. Model karşılaştırma grafikleri
# 
# Her veri seti için ROC-AUC değerleri çizilir.  
# Bu grafik, modellerin genel ayırma gücünü hızlı karşılaştırmak için kullanılır.

# %%
for dataset_key in metrics_df["Dataset"].unique():
    sub = metrics_df[metrics_df["Dataset"] == dataset_key].sort_values("ROC", ascending=True)

    plt.figure(figsize=(9, 5))
    plt.barh(sub["ModelType"], sub["ROC"])
    plt.xlabel("ROC-AUC")
    plt.title(f"{dataset_key} — 10 model karşılaştırması")
    plt.tight_layout()
    plt.savefig(lesson_out / f"{dataset_key}_model_comparison_roc.png", dpi=300, bbox_inches="tight")
    plt.show()
