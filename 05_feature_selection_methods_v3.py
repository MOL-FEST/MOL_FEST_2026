#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Ders 3A — Feature Selection Yöntemleri

# %% [markdown]
# # Ders 3A — Feature Selection Yöntemleri
# 
# ANOVA, chi-square, mutual information ve RF importance yöntemleriyle top-k feature selection yapılır.
...
# %% [markdown]
# ## 7. Feature selection yöntemleri
# 
# Feature selection, çok sayıda feature içinden model için daha bilgi taşıyanları seçmeyi amaçlar.  
# Bu bölümde aynı Random Forest modeli farklı feature seçim yöntemleriyle karşılaştırılır.

# %%
from sklearn.feature_selection import f_classif, chi2, mutual_info_classif

lesson_out = ensure_output_root() / "05_feature_selection"
lesson_out.mkdir(parents=True, exist_ok=True)

loaded = load_all_datasets()
features = build_features_for_all(loaded, feature_set="maccs_morgan")
...
for dataset_key, data in features.items():
    ...
    k_values = [min(k, len(names)) for k in K_VALUES]

    rankers = {}

    F, _ = f_classif(X_train, y_train)
    rankers["ANOVA"] = np.argsort(np.nan_to_num(F))[::-1]

    chi_scores, _ = chi2(X_train, y_train)
    rankers["Chi2"] = np.argsort(np.nan_to_num(chi_scores))[::-1]

    mi = mutual_info_classif(X_train, y_train, discrete_features="auto", random_state=RANDOM_STATE)
    rankers["MutualInfo"] = np.argsort(np.nan_to_num(mi))[::-1]

    rf_selector = make_rf()
    rf_selector.fit(X_train, y_train)
    rankers["RF_importance"] = np.argsort(rf_selector.feature_importances_)[::-1]

    for method, order in rankers.items():
        ranking = pd.DataFrame({"Feature": [names[i] for i in order], "Rank": np.arange(1, len(order) + 1)})
        ranking.to_csv(lesson_out / f"{data['info']['model_prefix']}_{method}_ranking.csv", index=False)
        ...
        for k in k_values:
            ...
            y_score = get_score_class1(model, X_test[:, idx])
            metrics = calculate_metrics(y_test, y_pred, y_score)

            metrics.update({"Dataset": dataset_key, "Method": method, "K": k})
            rows.append(metrics)
            print(f"{dataset_key} | {method} top{k}: ROC={metrics['ROC']:.3f}, F1={metrics['F1']:.3f}")

result_df = pd.DataFrame(rows).sort_values(["Dataset", "ROC"], ascending=[True, False])
result_df.to_csv(lesson_out / "feature_selection_performance_both_datasets.csv", index=False)
display(result_df)

# %%
for dataset_key in result_df["Dataset"].unique():
    sub = result_df[result_df["Dataset"] == dataset_key]

    plt.figure(figsize=(10, 5))
    for method in sub["Method"].unique():
        s = sub[sub["Method"] == method].sort_values("K")
        plt.plot(s["K"], s["ROC"], marker="o", label=method)

    plt.xlabel("Seçilen feature sayısı")
    plt.ylabel("ROC-AUC")
    plt.title(f"{dataset_key} — Feature selection performansı")
    plt.legend()
    plt.tight_layout()
    plt.savefig(lesson_out / f"{dataset_key}_feature_selection_roc.png", dpi=300, bbox_inches="tight")
    plt.show()
