#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Ders 2B — Cross-validation ve Model Stabilitesi

# %% [markdown]
# # Ders 2B — Cross-validation ve Model Stabilitesi
# 
# 5-fold cross-validation ile modellerin farklı veri bölünmelerindeki stabilitesi incelenir.
...
# 
# Tek train/test split sonucu şansa bağlı olabilir.  
# 5-fold cross-validation, modelin farklı bölünmelerde benzer performans verip vermediğini gösterir.
...
def cv_models():
    return {
        "LogisticRegression": Pipeline([("variance", VarianceThreshold(0.0)), ("scaler", StandardScaler()), ("model", LogisticRegression(max_iter=2000, class_weight="balanced", solver="liblinear"))]),
        "kNN": Pipeline([("variance", VarianceThreshold(0.0)), ("scaler", StandardScaler()), ("model", KNeighborsClassifier(n_neighbors=7, weights="distance"))]),
        "RandomForest": Pipeline([("variance", VarianceThreshold(0.0)), ("model", make_rf())]),
        "ExtraTrees": Pipeline([("variance", VarianceThreshold(0.0)), ("model", ExtraTreesClassifier(n_estimators=300, class_weight="balanced", random_state=RANDOM_STATE, n_jobs=-1))]),
        "GaussianNB": Pipeline([("variance", VarianceThreshold(0.0)), ("model", GaussianNB())]),
    }

lesson_out = ensure_output_root() / "04_cross_validation"
...
for dataset_key, data in features.items():
    for model_label, model in cv_models().items():
        model_name = f"{data['info']['model_prefix']}_{model_label}_5CV"
        note(f"5-fold CV: {model_name}")

        result = cross_validate(model, data["X"], data["y"], cv=cv, scoring=scoring, n_jobs=-1)
        row = {"Dataset": dataset_key, "ModelName": model_name, "ModelType": model_label}
        ...
        for metric in scoring:
            ...
            for fold, value in enumerate(values, start=1):
                fold_rows.append({"Dataset": dataset_key, "ModelType": model_label, "Fold": fold, "Metric": metric, "Value": value})

        summary_rows.append(row)
        print(f"ROC={row['ROC_mean']:.3f} ± {row['ROC_sd']:.3f}")

summary_df = pd.DataFrame(summary_rows).sort_values(["Dataset", "ROC_mean"], ascending=[True, False])
fold_df = pd.DataFrame(fold_rows)

summary_df.to_csv(lesson_out / "cv_summary_both_datasets.csv", index=False)
fold_df.to_csv(lesson_out / "cv_fold_metrics_long.csv", index=False)
display(summary_df)

# %%
for dataset_key in summary_df["Dataset"].unique():
    sub = summary_df[summary_df["Dataset"] == dataset_key].sort_values("ROC_mean", ascending=True)

    plt.figure(figsize=(9, 5))
    plt.barh(sub["ModelType"], sub["ROC_mean"], xerr=sub["ROC_sd"])
    plt.xlabel("ROC-AUC mean ± SD")
    plt.title(f"{dataset_key} — Cross-validation stabilitesi")
    plt.tight_layout()
    plt.savefig(lesson_out / f"{dataset_key}_cv_roc_mean_sd.png", dpi=300, bbox_inches="tight")
    plt.show()
