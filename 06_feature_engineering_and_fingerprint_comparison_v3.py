#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Ders 3B — Feature Engineering ve Fingerprint Karşılaştırması

# %% [markdown]
# # Ders 3B — Feature Engineering ve Fingerprint Karşılaştırması
# 
# MACCS, Morgan, RDKit descriptor ve birleşik feature setleri Random Forest ile karşılaştırılır.
...
# %% [markdown]
# ## 7. Feature engineering karşılaştırması
# 
# Bu bölümde model sabit tutulur, molekül temsil biçimi değiştirilir.  
# Amaç, performansın sadece modelden değil feature temsilinden de etkilendiğini göstermektir.

# %%
lesson_out = ensure_output_root() / "06_feature_engineering"
lesson_out.mkdir(parents=True, exist_ok=True)

loaded = load_all_datasets()
feature_sets = ["maccs", "morgan", "rdkit", "maccs_morgan", "morgan_rdkit"]

rows = []

for dataset_key, raw_data in loaded.items():
    for feature_set in feature_sets:
        note(f"{dataset_key} — feature set: {feature_set}")
        ...
        try:
            ...
            metrics.update({"Dataset": dataset_key, "FeatureSet": feature_set, "n_features": X.shape[1]})
            rows.append(metrics)

            print(f"Feature sayısı={X.shape[1]} | ROC={metrics['ROC']:.3f}, AP={metrics['AP']:.3f}")
        except Exception as e:
            print(f"{dataset_key} | {feature_set} çalışmadı: {e}")

result_df = pd.DataFrame(rows).sort_values(["Dataset", "ROC"], ascending=[True, False])
result_df.to_csv(lesson_out / "feature_engineering_comparison_both_datasets.csv", index=False)
display(result_df)
...
for dataset_key in result_df["Dataset"].unique():
    ...
    plt.barh(sub["FeatureSet"], sub["ROC"])
    plt.xlabel("ROC-AUC")
    plt.title(f"{dataset_key} — Feature engineering karşılaştırması")
    plt.tight_layout()
    plt.savefig(lesson_out / f"{dataset_key}_feature_engineering_roc.png", dpi=300, bbox_inches="tight")
    plt.show()
