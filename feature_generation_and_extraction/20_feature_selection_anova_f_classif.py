#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Supervised univariate selection with ANOVA F-test."""
...
K_VALUES=[50,100,150,200]; OUTDIR='feature_outputs/20_anova_f_classif'
...
def main():
    ...
    F,p=f_classif(X.values,y); F=np.nan_to_num(F,nan=0.0,posinf=0.0,neginf=0.0); p=np.nan_to_num(p,nan=1.0,posinf=1.0,neginf=1.0)
    ranking=pd.DataFrame({'Feature':cols,'F_score':F,'p_value':p}).sort_values('F_score',ascending=False).reset_index(drop=True)
    ranking['Rank']=np.arange(1,len(ranking)+1)
    kvalues=k_list(len(cols),K_VALUES); selected={k: ranking['Feature'].head(k).tolist() for k in kvalues}
    save_selection(outdir,'anova_f_classif',df,X,y,target,smiles,ranking,selected)
