...
# %% [markdown]
# ## 7. İki veri setinin genel yapısını inceleyelim
# 
# Model kurmadan önce verinin boyutu ve sınıf dağılımı kontrol edilir.  
# Sınıf dağılımı özellikle recall, precision ve balanced accuracy gibi metrikleri doğrudan etkiler.
...
for dataset_key, data in loaded.items():
    ...
    plt.tight_layout()
    plt.savefig(lesson_out / f"{info['model_prefix']}_class_distribution.png", dpi=300, bbox_inches="tight")
    plt.show()
...
summary_df.to_csv(lesson_out / "dataset_summary.csv", index=False)
display(summary_df)

# %% [markdown]
# ## 8. İki veri seti için fingerprint dosyaları üretelim
# 
# Bu aşamada her iki veri seti için MACCS + Morgan fingerprint üretilir.  
# Bu dosyalar sonraki modellerde doğrudan kullanılabilecek sayısal feature tablolarıdır.
...
for dataset_key, data in features.items():
    ...
    feature_df = pd.concat(
        [
            data["df"][[data["smiles_col"], data["target_col"]]].reset_index(drop=True),
            pd.DataFrame(data["X"], columns=data["feature_names"])
        ],
        axis=1
    )

    out_file = lesson_out / f"{info['model_prefix']}_maccs_morgan_features.csv"
    feature_df.to_csv(out_file, index=False)

    print(f"{info['short_name']} için feature dosyası hazır.")
    print(f"Dosya: {out_file}")
    print(f"Satır: {feature_df.shape[0]}")
    print(f"Toplam kolon: {feature_df.shape[1]}")
    print(f"Feature sayısı: {len(data['feature_names'])}")
